...
const API_KEY = 'API-KEY-SENTINELA-HEALTH';
const WEBHOOK_SECRET = 'WEBHOOK-SECRET-SENTINELA-HEALTH';
...
function request() {
  return new Request(
    `http://localhost:3000/api/platform/tenants/${TENANT}/channels/${CONNECTION}/healthcheck`,
    { method: 'POST' },
  );
}
...
beforeEach(() => {
  ...
  fetchEvolutionConnectionStateMock.mockResolvedValue({
    stateLabel: 'open',
...
describe('POST healthcheck', () => {
  it('devolve somente o resultado público e usa select mínimo', async () => {
    ...
    expect(body.healthcheck).toMatchObject({ state: 'open' });
    expect(persistedUpdate).toMatchObject({ status: 'connected' });
    expect(persistedUpdate?.metadata).toMatchObject({
      lastHealthcheckRaw: { state: 'open', nested: {} },
    });
  ...
  it('não devolve a mensagem do resolvedor quando ela contém segredo externo', async () => {
    const agencySecret = 'AGENCY-SECRET-NOT-IN-CONNECTION-HEALTH';
    ...
    expect(body.error).toBe('Failed to resolve Evolution credentials for healthcheck.');
  ...
  it('redige segredos ecoados no erro do provedor', async () => {
    fetchEvolutionConnectionStateMock.mockRejectedValue(
      new Error(`health apiKey=${API_KEY} webhookSecret=${WEBHOOK_SECRET}`),
    );
