...
| **I7** segredo do webhook na query string (G10/G11/G22) | **Confirmado e corrigido** | A Evolution aceita `webhook.headers` (conferido em `webhook.controller.ts:41,50` e `webhook.schema.ts:34` do repositório). `registerCrmWebhook` (`lib/channels/evolutionWebhookRegistration.ts`), usado por connect e healthcheck: registra a URL LIMPA com o segredo no cabeçalho `x-webhook-secret`, confere com `GET /webhook/find` que a Evolution guardou o cabeçalho e, só se não guardou (versão antiga), registra de novo com `?secret=` e devolve aviso; `lastWebhookTransport` (`header`/`query`) fica na metadata. O leitor do segredo (`readWebhookSecretFromRequest`, compartilhado por webhook e ai-reply) prefere cabeçalho → Bearer → query (query só para registros antigos). Rotação: o healthcheck da tela re-registra (em produção é alteração: só com o "pode"). HMAC do corpo + timestamp continuam na SPEC §7 |
| **I8** Next `16.2.12` sob 2 advisories críticos (G12) | **Confirmado e corrigido** | `npm audit` em 14/09: 18 (1 crítica, 5 altas, 5 médias, 7 baixas), todas com correção sem subir versão maior. `npm audit fix` levou Next a **16.3.5**, Vitest 4.1.11, Faker 10.6, AI SDK 6.0.283 (só lockfile). Sobravam 2 altas do `sharp`: o `overrides` do `package.json` fixava `0.35.0` desde 03/08 (`01e0fd9`) e o Next 16.3.5 pede `^0.35.4`; a trava subiu para `0.35.4`. **`npm audit` = 0.** A RCE por AVIF era pouco explorável aqui (imagens `unoptimized`, sem `remotePatterns`), corrigida mesmo assim. Provas: `tsc` limpo; `next build` com o Next 16.3.5 compila e gera as 110 páginas (1 aviso pré-existente do instalador, `lib/installer/edgeFunctions.ts:858`, acesso dinâmico ao sistema de arquivos, não bloqueia); suíte completa **257 arquivos / 1.287 testes, zero falhas** (231 s), lida em comando separado antes do commit |
| **S5** erro interno devolvido pela rota admin (G10) | **Confirmado e corrigido** | `settings/automations-live` devolvia `error.message` do banco em 4 pontos; agora devolve uma mensagem estável em linguagem leiga e grava o detalhe (código + mensagem) só no log do servidor. Exceção mantida: o gate de saúde (`55000`), cuja mensagem é escrita para o usuário na própria função. Teste estático em `funilLiveMigration.test.ts` |
| **B1 / G3 / G11 / G13** webhook legado fail-open (raiz) | **Confirmado e corrigido** | `evaluateWebhookAuth` devolvia autorizado para QUALQUER POST quando a conexão não tinha `webhookSecret` (nem conferia o `instanceName`); um inbound forjado disparava a IA pelo número do cliente, resolvia esperas e criava marco para a Meta. Agora é fail-closed: só entra com o segredo da conexão; sem segredo → 401 com mensagem leiga ("abra a tela de conexões e gere o QR code de novo"). Migration `20260915000000` preenche o segredo em toda conexão sem ele (só onde vazio; nunca troca). Lido em produção em 15/09: a única conexão (Jéssica) já tinha segredo, então nada muda lá. `ai-reply` passou a comparar o segredo em tempo constante |
...
| Achados meus além do parecer | **Corrigidos** | Cabeçalhos de segurança em toda resposta (`next.config`): `nosniff`, `X-Frame-Options: DENY` + `frame-ancestors 'none'`, `Referrer-Policy`, `Permissions-Policy` (câmera/geolocalização/pagamento fechados, microfone só na própria origem) e HSTS explícito; CSP completa fica como dívida (exige nonce para os scripts inline). `ai-reply` compara o segredo em tempo constante (feito no `7af8268`). Teste `test/securityHeaders.test.ts` |

**Provas desta resposta (S5 + cabeçalhos):** `test/funilLiveMigration.test.ts` (estático da rota admin: mensagem estável presente, `error.message` fora do 55000 ausente) · `test/securityHeaders.test.ts` 2/2 · `tsc`/ESLint limpos · `next build` compila e gera as 110 páginas (mesmo aviso pré-existente do instalador) · suíte completa **258 arquivos / 1.289 testes, zero falhas** (217 s; +1 arquivo / +2 testes = o teste dos cabeçalhos), lida em comando separado antes do commit.

**Provas desta resposta (webhook, I7 + B1):** `lib/conversations/webhookAuth.test.ts` 8/8 (sem segredo → recusa mesmo
com instanceName batendo; leitor do segredo: cabeçalho, Bearer, query legado, cabeçalho vence a URL) ·
`lib/channels/evolutionWebhookRegistration.test.ts` 4/4 (URL limpa + cabeçalho; Evolution antiga cai para a URL com aviso;
eco parcial não conta; erro sobe) · `test/webhookSegredoMigration.test.ts` 2/2 (um só UPDATE, só onde vazio, aditiva) ·
rotas connect/healthcheck com a expectativa nova (URL sem `secret=`, cabeçalho, `lastWebhookTransport`) · migration
aplicada no local (ledger `20260915000000`; 0 conexões sem segredo) · suíte completa: **257 arquivos / 1.287 testes, zero falhas** (237 s). Eram 255 / 1.277: os +2 / +10 são exatamente os
testes novos (autenticação +4, registro 4, migration 2). Saída em arquivo, lida em comando separado antes do commit.
