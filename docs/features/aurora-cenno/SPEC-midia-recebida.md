...
## Janela 4 (21/09/2026): o que o teste real mostrou e o que mudou

Teste prático no número real (15h47 às 17h52, gates desligados às 17h52min36s), com o Junior e a Rayanne. Leitura do Junior: "ela se saiu bem, identificou as mensagens corretamente, fizemos teste com barulho e ela não errou nada".

- **Visto funcionando:** áudio transcrito e respondido em 16 a 18 s; áudio com barulho sem erro; áudio mudo classificado como `empty` com aviso no sino; figurinha e cartaz descritos (texto visível entre «»); vídeo só com selo e aviso; e-mail ditado não gravado; mensagens fora de contexto tratadas; cutucada de inatividade.
- **Falha compreensível 1 (sem mudança por ora):** a Rayanne mandou vídeo no lugar da imagem, depois "Um pouco do nosso evento" e uma figurinha de nave; a Aurora ligou o texto à figurinha ("evento bacana esse do disco voador") e disse que não abriu o vídeo. No payload a "GIF" chegou como figurinha estática (`stickerMessage`), por isso foi descrita; GIF de verdade (vídeo com `gifPlayback`) só teria selo. Ajuste possível, só com o "vai" do Junior: regra dizendo que figurinha é reação e não foto do negócio, e que "isso/esse" logo depois de um vídeo não visto fala do vídeo.
- **Falha 2, áudio mudo sem resposta — NÃO era da mídia:** corrida de gravação em `conversation_threads.metadata`, em código compartilhado (também na `main`). Vários gravadores leem a metadata inteira e a regravam a partir de uma foto antiga; quando a foto é anterior à gravação do `aiPendingToken` da mensagem nova, o marcador volta para o de uma mensagem já respondida e `processDeferredAIReply` sai calado. Provado pelos logs de borda do Supabase em dois casos do mesmo teste: a tela do CRM marcando como lida (leu 3 ms antes do webhook gravar, gravou 77 ms depois) e o envio da própria IA gravando a foto lida antes de enviar (3 s). A imagem das 16h12min30s também ficou sem resposta própria pelo mesmo motivo. Um terceiro gravador com o mesmo formato é o envio manual pela tela. **Conserto:** `20260921030000_conversation_thread_keep_newest_ai_pending_token.sql`, gatilho `BEFORE UPDATE OF metadata` que mantém o marcador de maior carimbo quando uma escrita traz um mais antigo, malformado ou nenhum (o resto da escrita passa). Nenhum fluxo limpa o marcador de propósito (mapa de todos os gravadores, só leitura). Sem mudança de código no app. Aplicado e provado no preview (7 casos, inclusive como `service_role`, conversa temporária apagada e conferida por contagem). Produção só com o "pode".
- **Pedido do Junior, e-mail ditado (implementado):** em vez de só pedir para digitar, a Aurora escreve o e-mail como entendeu e pergunta "você disse que seu e-mail é X, está certo?"; se o lead confirmar, grava; se não, "faz assim, [nome], me manda por escrito pra eu não errar e marcar com o e-mail errado". A trava em código continua (`resolveConfirmedLeadEmail`, no lugar de `hasInboundAudioSinceLastReply ? null`): com áudio do lead desde a última resposta, se ele DIGITOU algum e-mail desde então vale só o último e-mail da mensagem digitada mais recente (em "não é X, é Y" vale Y); senão vale o ÚNICO e-mail que a última resposta do CRM escreveu para ele conferir (duas opções escritas = ambíguo, não grava); e-mail que só existe na transcrição nunca é gravado, porque o primeiro e-mail gravado nunca é trocado (`leadProfile.ts`). Revisão adversarial (3 lentes, só leitura) pegou a correção digitada com os dois e-mails e o apóstrofo (`o'brien@`); ambos consertados e cobertos por teste. **Fronteira consciente:** se o lead der a entender "não" por áudio e o modelo mesmo assim devolver o e-mail da conferência, passa; decidir se houve confirmação é do modelo (desenho pedido), o código barra só o e-mail sem rastro escrito. Resíduo conhecido e anterior a esta mudança: o histórico é ordenado só por `sent_at` (o do lead tem resolução de segundo), então um empate no mesmo segundo pode embaralhar a fronteira "antes/depois da última resposta"; raríssimo, não tratado.
- **Achados paralelos, fora desta obra (decidir à parte):** a escrita final de `executeConversationAIReply` (e o próprio webhook de mensagem nova) não tem guarda de status e pode devolver para `ai_active` uma conversa que o operador acabou de assumir; a mesma foto velha pode ressuscitar uma cutucada cancelada; `recordConversationAIFailure` grava `human_queue` sem guarda. E, na janela no número real, a IA atende lead real ao mesmo tempo que o Junior (a Alexandra teve a reunião confirmada pela Aurora enquanto ele marcava no Meet; bateu por coincidência): sugestão de trava de ensaio por lista de telefones de teste.

### Lote antes da produção (21/09/2026)

Respostas do Junior aos pontos em aberto: regra da figurinha "ok"; trava de ensaio por lista de telefones "não precisa"; guarda do "assumir" "ok"; produção "pode". Implementado:

- **Regra da figurinha** (`INBOUND_MEDIA_AI_RULES`): figurinha e GIF são reação, não foto do negócio do lead; texto que aponta para algo ("esse é o evento") fala da mídia enviada junto dele que não seja figurinha nem GIF; se essa mídia não foi vista, a IA diz que não abriu e pede para o lead contar.
- **Nada atropela o "assumir"**: a escrita final de `executeConversationAIReply` só grava se o status da conversa ainda é o que foi lido antes do envio (a do encerramento já exigia `human_queue`); se o operador assumiu ou resolveu durante o envio, o estado dele fica e quem chama recebe o status real. `recordConversationAIFailure` só move para a fila a conversa que ainda estava com a IA (`open`, `ai_active` ou já em `human_queue`): quem o humano assumiu ou resolveu no meio do caminho fica como está, e o sino só avisa quando a conversa foi de fato para a fila (revisão adversarial `wf_39e463de-212`). O histórico que a IA lê ganhou desempate pela hora de gravação (`sent_at` do lead tem resolução de segundo; a regra da figurinha e a trava do e-mail dependem da ordem). Toda resposta da IA limpa a cutucada pendente (a do silêncio novo é agendada logo depois), então a foto antiga não ressuscita cutucada cancelada. Teste com banco falso simulando as duas corridas (`aiReply.threadStateGuard.test.ts`); o banco falso passou a devolver cópia na leitura, como o de verdade.
- Fora da mídia, no mesmo lote: o nome "Julia" saiu do CRM (textos de tela viram "IA"; nome padrão do agente vira "Assistente"; o prompt padrão não dá nome à IA); os 8 "Base CRM" restantes do rebrand viraram "CENNO CRM"; fundo escuro em preto profundo (emenda em `docs/design/tema-cenno.md`).
- Não tratado aqui (registrado): a mesma corrida existe no webhook de mensagem recebida entre ler e gravar o status (janela de uma ida e volta ao banco, sem espera no meio) e no envio manual pela tela para as outras chaves da metadata; o gatilho `20260921030000` já protege o marcador da resposta nesses caminhos.
