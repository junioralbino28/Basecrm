...
| **Backlog aceito (2026-07-24): Visão Geral com widgets CONFIGURÁVEIS + visões que faltam.** Pedido do Junior durante o C2C: (a) **ganho por procedimento** — qual procedimento traz mais/menos receita (dado JÁ existe: etiqueta principal do deal + valor; e atendimentos têm procedimento+valor); (b) **desempenho por colaborador** — agendamentos e fechamentos por usuário, quem converte mais (parcial já existe: leaderboard de vendedor em Relatórios; falta agendamento e trazer pro dashboard); (c) **acompanhamento de oportunidades**; (d) admin escolhe QUAIS cards a empresa vê (personalização por org, casa com o `branding_config`). Ordem: junto das consolidações pós-C2D, na mesma leva do financeiro v2/relatórios. Nichos além de clínica: "procedimento" = produto/serviço da etiqueta — manter neutro. | pedido do Junior, 2026-07-24 |
| **Branding por cliente (aprovado): motor e base FIXOS, casca personalizável.** A **agência** define cor primária + logo na **ativação** do cliente; o nome já vem da organização. Viável barato porque a escala `brand-50..900` é variável CSS única (`globals.css @theme`) — tema por tenant = injetar valores por org. Entra **pós-C2D** (~1-2 dias) e começa por uma **pesquisa profunda de mercado**: como (e SE) os CRMs entregam branding/white-label — doc `PESQUISA-BRANDING-CRMS.md`. White-label completo (domínio por cliente, e-mails com a marca) fica fora deste escopo. NÃO confunde com a tela self-service de personalização (segue backlog pós-100%). | decisão do Junior, 2026-07-24 |
| **Toda mensagem de erro ou instrução visível fala linguagem LEIGA** (pedido do Junior 2026-07-24, ao receber "config inválida em switch: cases.0.value operador contains exige value"). Erro técnico do motor não chega cru na tela: a UI traduz (padrão: `features/automations/automationErrorMessages.ts` — diz qual passo, o que falta e o que fazer, com o detalhe técnico entre parênteses pro suporte). Vale pra TODA tela nova. | pedido do Junior, 2026-07-24 |
...
| **Competência da comissão = `performed_at`; caixa = `paid_at`.** A comissão é devida integralmente no mês do atendimento, mesmo sem recebimento do cliente. Mudança de regra não reescreve atendimento passado. | [DECISOES-JUNIOR-FINANCEIRO-29-07.md](./DECISOES-JUNIOR-FINANCEIRO-29-07.md) + implementação em [IMPL-LOG-CORRECOES-PACOTE-3.md](./IMPL-LOG-CORRECOES-PACOTE-3.md) |
| **Remuneração fixa e híbrida são cálculo, não cadastro informativo.** Fixo é versionado por data e rateado por dias; comissão é calculada por evento; `both` soma as duas parcelas. Contrato somente `fixed` nunca recebe comissão. | mesmos documentos do Pacote 3 |
| **Especialidade é arquivada, não apagada pelo cliente.** O fluxo normal é `active=false`; `authenticated` não possui `DELETE/TRUNCATE`, preservando vínculos e histórico. | `86e6d60` · `20260803100000_p3_specialty_archive_guard.sql` |
...
| **Serviço financeiro recebe o tenant ativo explicitamente e falha fechado sem ele.** Não inferir a organização pelo perfil nem mutar somente por ID; leitura e escrita filtram `organization_id`. | `86e6d60` · [IMPL-LOG-CORRECOES-PACOTE-3.md](./IMPL-LOG-CORRECOES-PACOTE-3.md) |
...
| **Pacote 3 permanece sem deploy até reverificação independente.** “Correções implementadas e testes locais verdes” não significa “aprovado para produção”; o próximo parecer será `docs/REVIEW-CORRECOES-PACOTE-3.md`. | handoff de 2026-08-03 |
